...
-- CẤU HÌNH TỐI ƯU
local Settings = {
    Radius = 15, -- Khoảng cách giữ với kẻ địch (xa hơn để né chiêu gần)
    Height = 10, -- Độ cao (đứng trên đầu khó bị đánh trúng hơn đứng dưới đất)
    Speed = 10,  -- Tốc độ xoay vòng quanh kẻ địch
    SafeMode = true -- Bật chế độ an toàn (tự lùi khi máu thấp)
}

spawn(function()
    while task.wait() do -- Dùng wait() ngắn nhất
        pcall(function()
            if _G.Defeating then
                -- Lọc tìm mục tiêu gần nhất và hợp lệ
                local Target = nil
                local ShortestDist = 250 -- Tầm quét

                for _, v in pairs(workspace.Characters:GetChildren()) do
                    if v.Name ~= Plr.Name and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                ...
                if Target then
                    local vRoot = Target.HumanoidRootPart
                    local vHum = Target.Humanoid
                    
                    repeat
                        RunService.Heartbeat:Wait() -- Chạy theo từng frame để mượt nhất
                        ...
                        local MyHum = Plr.Character.Humanoid

                        local Angle = tick() * Settings.Speed
                        local Offset = Vector3.new(math.cos(Angle) * Settings.Radius, Settings.Height, math.sin(Angle) * Settings.Radius)
                        
                        local TargetPos = vRoot.Position + Offset
                        
                        -- Sử dụng hàm _tp của bạn hoặc CFrame thường
                        if _tp then
                            _tp(CFrame.new(TargetPos, vRoot.Position)) -- CFrame nhìn vào địch
                        else
                            MyRoot.CFrame = CFrame.new(TargetPos, vRoot.Position)
                        end
                        
                        -- Reset vận tốc để không bị rơi hoặc bị đẩy
                        MyRoot.Velocity = Vector3.new(0,0,0) 

                        -- 2. LOGIC TẤN CÔNG (FAST KILL)
                        EquipWeapon(_G.SelectWeapon)
                        
                        -- Thêm logic Auto Click nếu game cần click chuột trái
                        game:GetService("VirtualUser"):CaptureController()
                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))

                        sethiddenproperty(Plr, "SimulationRadius", math.huge)

                    until _G.Defeating == false or vHum.Health <= 0 or not Target.Parent or not vRoot
                end
            end
        end)
    end
