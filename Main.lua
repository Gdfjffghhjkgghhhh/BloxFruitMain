...
local enemyPartsCache = setmetatable({}, { __mode = "k" })
local moving = {}
local lastRun = 0
local MIN_INTERVAL = 0.15 
local function tryExpandSimRadiusOnce()
    if _G._simulationAttempted then return end
    _G._simulationAttempted = true
    pcall(function()
        sethiddenproperty(plr, "SimulationRadius", math.huge)
    end)
end

local function collectParts(enemy)
    local parts = enemyPartsCache[enemy]
    ...
    enemyPartsCache[enemy] = parts
    return parts
end

local function restoreEnemy(enemy)
    ...
    if humanoid then
        ...
        local storedAnimator = humanoid:GetAttribute("StoredAnimator")
        if storedAnimator and not humanoid:FindFirstChildOfClass("Animator") then
            pcall(function() storedAnimator.Parent = humanoid end)
            humanoid:SetAttribute("StoredAnimator", nil)
        end
    end

    for _, part in ipairs(collectParts(enemy)) do
        if part and part.Parent then
            part.CanCollide = true
        end
    end

    enemy:SetAttribute("BeingBrought", nil)
    moving[enemy] = nil
end
RunService.Heartbeat:Connect(function(dt)
    if next(moving) == nil then return end
    for enemy, data in pairs(moving) do
        if not enemy or not enemy.Parent then
            ...
            if not humanoid or not hrp or humanoid.Health <= 0 then
                ...
                local toTarget = data.target - hrp.Position
                local dist = toTarget.Magnitude
                if dist <= 3 then
                    restoreEnemy(enemy)
                else
                    local step = data.speed * dt
                    local alpha = math.min(1, step / (dist ~= 0 and dist or 1))
                    local newPos = hrp.Position:Lerp(data.target, alpha)
...
function BringEnemy()
    ...
    if tick() - lastRun < MIN_INTERVAL then return end
    lastRun = tick()
    ...
    for i = 1, #enemies do
        ...
        if hrp and humanoid and humanoid.Health > 0 then
            totalpos[v.Name] = (totalpos[v.Name] or Vector3.new()) + hrp.Position
            counts[v.Name] = (counts[v.Name] or 0) + 1
        end
    end
    for name, pos in pairs(totalpos) do
        totalpos[name] = pos / counts[name]
    end

    tryExpandSimRadiusOnce()
    ...
    for i = 1, #enemies do
        ...
        if v and v.Parent then
            ...
            if humanoid and hrp and humanoid.Health > 0 then
                if (hrp.Position - PosMon).Magnitude <= 350 and not v:GetAttribute("BeingBrought") then
                    local mobPos = totalpos[v.Name]
                    if mobPos then
                        local distance = (hrp.Position - mobPos).Magnitude
                        if distance > 3 and distance <= 180 then
                            v:SetAttribute("BeingBrought", true)
                            if humanoid:GetAttribute("DefaultWalkSpeed") == nil then
                                humanoid:SetAttribute("DefaultWalkSpeed", humanoid.WalkSpeed)
                            end
                            if humanoid:GetAttribute("DefaultJumpPower") == nil then
                                humanoid:SetAttribute("DefaultJumpPower", humanoid.JumpPower)
                            end
                            local parts = collectParts(v)
                            ...
                            if animator then
                                humanoid:SetAttribute("StoredAnimator", animator)
                                pcall(function() animator.Parent = nil end)
                            end
                            local minTime = 0.15
                            local maxTime = 1.5
                            local tweenTime = math.clamp(distance / 350, minTime, maxTime)
                            local speed = distance / tweenTime
                            moving[v] = { target = mobPos, speed = speed }
                            local diedConn
	...
	while task.wait(0.15) do
		if _G.AutoFarm_Bone then
          ...
          {mob = "Living Zombie",   quest = {"StartQuest", "HauntedQuest1", 2}},
				...
				local function GetAliveMobs(mobName)
					local mobs = {}
					for _, v in pairs(workspace.Enemies:GetChildren()) do
						if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
				...
				local function FarmAllInArea(mobData)
					while task.wait() and _G.AutoFarm_Bone do
						local mobs = GetAliveMobs(mobData.mob)
						if #mobs == 0 then
							break 
						end

						for _, bone in ipairs(mobs) do
							if not _G.AutoFarm_Bone then break end
							if bone:FindFirstChild("HumanoidRootPart") then
								if _G.AcceptQuestC and not questUI.Visible then
									_tp(QuestPos)
									repeat task.wait(0.2) until (QuestPos.Position - root.Position).Magnitude <= 60
