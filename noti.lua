    ...
    while task.wait(0.15) do
        if _G.AutoFarm_Bone then
            pcall(function()
                ...
                -- Kiểm tra nếu ở quá xa khu vực farm
                if (root.Position - QuestPos.Position).Magnitude > 1000 then
                    _tp(QuestPos)
                    repeat task.wait(0.3) until (root.Position - QuestPos.Position).Magnitude <= 80 or not _G.AutoFarm_Bone
                end
                ...
                for _, mobData in ipairs(BonesTable) do
                    ...
                    while _G.AutoFarm_Bone do
                        ...
                        for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                            if enemy.Name == mobData.mob and enemy:FindFirstChild("HumanoidRootPart") and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
                                table.insert(allEnemies, enemy)
                            end
                        end

                        if #allEnemies == 0 then break end
                        ...
                        for _, bone in ipairs(allEnemies) do
                            if not _G.AutoFarm_Bone then break end
                            if bone and bone.Parent and bone:FindFirstChild("HumanoidRootPart") and bone.Humanoid.Health > 0 then
                                PosMon = bone.HumanoidRootPart.Position

                                repeat
                                    task.wait()
                                    
                                    -- Bypass TP: Di chuyển trực tiếp đến quái
                                    local targetPos = bone.HumanoidRootPart.Position
                                    root.CFrame = CFrame.new(targetPos + (bone.HumanoidRootPart.CFrame.lookVector * 4))
                                    
                                    BringEnemy()
                                    Attack.Kill(bone, _G.AutoFarm_Bone)
                                    
                                    -- Chờ một chút để tránh spam teleport
                                    task.wait(0.3)
